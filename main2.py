...
@app.get('/getusers')
def getusers(token: str = Depends(JWTBearer()), session: Session = Depends(get_session)):
...
def logout(token: str = Depends(JWTBearer()), db: Session = Depends(get_session)):
    ...
    user_id = payload['sub']
    token_record = db.query(models.TokenTable).filter(models.TokenTable.user_id == user_id).all()
    info = []
